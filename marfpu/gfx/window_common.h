...
static int common_setup_gl(gfx_common_window* self) {
	// OpenGL stuff
...
static void force_button(gfx_common_window* self, int id, int down, uint32_t character, int collect_event) {
...
static void set_button(gfx_common_window* self, int id, int down, uint32_t character, int collect_event) {
	if((id < kbRangeBegin || id > numButtons)
	|| self->buttonState[id] != down)
