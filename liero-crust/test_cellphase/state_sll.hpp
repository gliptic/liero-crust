...
#include "../lierosim/cellphase_sll.hpp"
...
namespace test_cellphase {
	...
	typedef FixedObjectList<ObjSll, ObjLimit, true> ObjListSll;

	struct StateSll : BaseState {


		ObjListSll objs;
		CellphaseSll obj_broadphase;
		...
		void process() {
			{
				obj_broadphase.clear();

				auto r = objs.all();

				for (ObjSll* b; (b = r.next()) != 0; ) {
					...
					if (new_state != ObjState::Alive) {
						...
						obj_broadphase.insert(CellNode::Index(objs.index_of(b)), b->pos);
					}
				}
			}

			auto new_nobjs = objs.flush_new_queue([&](ObjSll*, u32 index) {
		...
		inline bool obj_timed_out(ObjSll& self);
	};

	inline bool StateSll::obj_timed_out(ObjSll& self) {
